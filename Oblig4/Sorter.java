...
class Sorter{
	...
	Sorter(String ned, String hay){
		...
		if(needle.length > haystack.length){
			...
			settHopp();
			// System.out.println("WIDLCARD: " + wildcard + "\n");
			finnNeedle();
		}
		printLos();
	}
 ...
 */
	void finnNeedle(){
		for (int i=needle.length-1;i<haystack.length; i++) {
			// System.out.println("Verdi til I: " + i);

			// System.out.println("SJekker " + needle[needle.length-1] + " med " + haystack[i] );
			if(needle[needle.length-1] == haystack[i] || needle[needle.length-1]== '_'){
				...
				if(pri.containsKey(haystack[i])){
					...
					// System.out.println("Hopper: " + wildcard + "plasser");
					i+=wildcard-1;

				}
			}
			// System.out.println("I verdi: " + i);
